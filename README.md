...
sequenceDiagram
    ...
    Backend->>Backend: jwt.verify(token) → decode sequence
    Backend->>Backend: SELECT registration_email_verification WHERE email
    ...
    Backend-->>User: ✅ 200 {success: true, code, token}
```
...
sequenceDiagram
    ...
    Backend->>Backend: SELECT students WHERE email (duplicate check)
    Backend->>Backend: SELECT registration_email_verification WHERE email
    Backend->>Backend: jwt.verify(token) → validate sequence & step=4
    Backend->>Backend: Confirm verified=true & not expired
    Backend->>Backend: bcrypt.hash(password, 10) → hashedPassword
    ...
    Backend-->>User: ✅ 201 {success: true, user: {id, email, name}}
```
...
sequenceDiagram
    ...
    Backend-->>User: ✅ 200 {success: true, result: [...departments]}
```
...
sequenceDiagram
    ...
    Backend->>Backend: bcrypt.compare(password, hashedPassword)
    Backend->>Backend: jwt.sign → refreshToken + accessToken
    Backend->>Backend: INSERT refreshtokens (role:"admin")
    Backend->>Backend: Set cookies: refreshToken, refreshTokenId, role
    Backend-->>User: ✅ 200 {token, role:"admin", data: adminData}
```
...
sequenceDiagram
    ...
    Backend-->>User: ✅ 200 {data: [sessions with device_info, timestamps]}
```
...
sequenceDiagram
    ...
    Backend-->>User: ✅ 200 "Session removed successfully"
```
...
sequenceDiagram
    ...
    Backend-->>User: ✅ 200 "Logged out successfully"
```
...
sequenceDiagram
    ...
    Backend-->>User: ✅ 200 "Admin logged out"
```
...
sequenceDiagram
    ...
    Backend-->>User: ✅ 200 "Password changed successfully"
```
...
sequenceDiagram
    ...
    Backend-->>User: ✅ 200 {data: [records], page, total, totalPages}
```
